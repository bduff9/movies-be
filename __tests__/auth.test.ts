...
describe('customAuthChecker', () => {
	it('allows read access to all', () => {
		const context = { userObj: null } as TCustomContext;
		const result = customAuthChecker({ args: {}, context, info: {} as any, root: {} }, [
			'reader',
		]);
	...
	it('allows editor access to signed in users', () => {
		const context = ({ userObj: { id: 1 } } as unknown) as TCustomContext;
		const result = customAuthChecker({ args: {}, context, info: {} as any, root: {} }, [
			'editor',
		]);
